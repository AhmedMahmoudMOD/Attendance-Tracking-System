...
namespace Attendance_Tracking_System.Controllers
{
    public class ReportController : Controller
    {
        ...
        public IActionResult GetRangeStaffAttendanceReport(int TypeNo, DateOnly Date,DateOnly EndDate)
        {
            List<object> data = new List<object>();
            switch (TypeNo)
            {
                case 1:
                    data = instructorRepo.GetForRangeAttendanceReport(Date,EndDate);
                    break;
                case 2:
                    data = employeeRepo.GetForRangeAttendanceReport(Date,EndDate);
                    break;
                default:
                    return BadRequest();
            }
            ...
            if (data != null)
            {
                ...
                graphics.DrawImage(image, 0, 0, 100, 100);
                PdfFont sfont = new PdfStandardFont(PdfFontFamily.Helvetica, 10);
                graphics.DrawString($"From : {Date}", sfont, PdfBrushes.Black, new PointF(440, 55));
                graphics.DrawString($"To :   {Date}", sfont, PdfBrushes.Black, new PointF(440, 70));
